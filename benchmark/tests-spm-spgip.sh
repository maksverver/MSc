...
run_on() {
	...
	do
		for alternate in '' -a
		do
			name=pgsolver-$(basename "$1" .pgsolver)$lift$alternate run \
			../run-test.sh -i pgsolver $lift $alternate --timeout=3600 --stats --verify "$1" --reorder shuffle
		done
	done
}
...
cd output-spm-spgip-shuffled-nodecM || exit 1
